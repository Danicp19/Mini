...
const localStorageString = localStorage.getItem("mini-app");
const localStorageData = localStorageString
  ? JSON.parse(localStorageString)
  : defaultData;
const data = reactive(localStorageData);
watch(data, (newData) => {
  localStorage.setItem("mini-app", JSON.stringify(newData));
});
interface Column {
  name: string;
  list: {
        ...
        <template #item="{ element: item, index: listIndex }">
          ...
          >
            ...
            <template v-else>
              ...
              >
                <button @click="item.editing = true">edi</button>
                <button @click="deleteItem(column, listIndex)">basu</button>
              </div>
